...
class Display:
    ...
    def plot_gsom_learning(self, new_node_map, labels, figure_id, title, output_filename, labels_alt=None):
        ...
        # colours, legend_patches = self._get_color_map_adl_activity()
        # colours, legend_patches = self._get_color_map_ped_behaviour()_get_color_map_afl
        # colours, legend_patches = self._get_color_map_learning(len(set(labels)))  # For topography measures
        colours, legend_patches = self._get_color_map_afl(labels)
        ...
        for key, value in self.gsom_node_map.items():
            ...
            if value.get_hit_count() > 0:
                ...
                if labels_alt is None:
                    ...
                    label_list = [str(labels_alt[lbl_id]) for lbl_id in value.get_mapped_labels()]
    ...
    @staticmethod
    def _get_color_map_afl(labels=None):
        np.random.seed(1)

        cmap = cm.get_cmap('Set1', max(labels)+1)
        # https://matplotlib.org/examples/color/colormaps_reference.html
